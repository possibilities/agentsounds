...
import { createAgentsoundsMcpServer } from "./mcp-server.ts";

/** stdout is exclusively JSON-RPC until EOF, transport close, or a signal. */
export async function serveAgentsoundsMcp(): Promise<void> {
  const { server, drain } = createAgentsoundsMcpServer();
